...
void Awg::convertIntTo12BitCharArray(const int* intArray, char* charArray, size_t numInts) {
    // 每8个int对应12个字节 (8 * 12 = 96 bits = 12 bytes)
    const size_t outputSize = (numInts * 12 + 7) / 8;

    // 初始化输出数组为0
    memset(charArray, 0, outputSize);

    for (size_t i = 0; i < numInts; ++i)
    {
        // 确保值在0-4095范围内 (12bit)
        uint16_t value = static_cast<uint16_t>(intArray[i] & 0xFFF);

        // 计算在输出数组中的位偏移
        size_t bitOffset = i * 12;
        size_t byteIndex = bitOffset / 8;
        int bitInByte = bitOffset % 8;

        // 将12bit值写入char数组的对应位置
        if (bitInByte <= 4) {
            // 值可以放在两个字节内
            uint16_t* twoBytePtr = reinterpret_cast<uint16_t*>(charArray + byteIndex);
            *twoBytePtr = (*twoBytePtr & ~(0xFFF << bitInByte)) | (value << bitInByte);
        } else {
            // 值跨越三个字节
            uint32_t threeByteValue =
                (static_cast<uint32_t>(charArray[byteIndex]) << 0) |
                (static_cast<uint32_t>(charArray[byteIndex + 1]) << 8) |
                (static_cast<uint32_t>(charArray[byteIndex + 2]) << 16);

            threeByteValue = (threeByteValue & ~(0xFFF << bitInByte)) | (value << bitInByte);

            charArray[byteIndex] = static_cast<unsigned char>(threeByteValue & 0xFF);
            charArray[byteIndex + 1] = static_cast<unsigned char>((threeByteValue >> 8) & 0xFF);
            charArray[byteIndex + 2] = static_cast<unsigned char>((threeByteValue >> 16) & 0xFF);
        }
    }
}

bool Awg::converteToBitFrame(const unsigned bitSize, char *frame, const Awg::DT *array, unsigned start, unsigned end)
{
    __m256i vec = _mm256_loadu_si256(reinterpret_cast<const __m256i*>(array));
    __m256i shiftMask = _mm256_setr_epi16(4,0,4,0,4,0,4,0,4,0,4,0,4,0,4,0);
}
